...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {  // A new announcement was submitted through the form
	...
	if (isset($_FILES['upload'])){
		$my_file_name = $_FILES['upload']['name'];
	} else {
		$my_file_name = "WTF Dude!";
	}
	
	echo '<div class="row">
			<div class="col-lg-12">
				<div class="alert alert-warning">
					<p align="center">SUBJECT: ' . $subject . '<br>
					                            BODY: ' . $body . '<br>
												FILE NAME: ' . $my_file_name . '</p>
				</div>
			</div>
		</div>';
	// ########## END DEBUG  ###############
	...
	if ($subject && $body) {  // Both are validated
		...
		if ($file_upload) { // A file was uploaded and validated
			...
			$file_path = ANNOUNCE_DIR . DIRECTORY_SEPARATOR . $file_name;
